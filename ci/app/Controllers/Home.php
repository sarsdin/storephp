...
//        log_message("debug", "noticeSearch:\$data: ",print_r($data));
        ...
        foreach ($this->testProps as $item) {
            if ($data['noticeSearchType'] == '제목') {
                //mb_strpos는 문자열에서 찾는 것이 없다면 false를 반환하고 있다면 문자열의 몇번째 인덱스(열)에 있는지 int를 반환한다
                if (mb_strpos($item['title'], $data['noticeSearch']) !== false ) {
                    array_push($res, $item);
                }
            } elseif ($data['noticeSearchType'] == '내용') {
                if (mb_strpos($item['hit'], $data['noticeSearch']) !== false) {
                    array_push($res, $item);
                }
            }
        }

        $res = $this->response->setJSON($res);
        return $res;
    }
